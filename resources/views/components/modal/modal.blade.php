...
>
    ...
    >
        ...
        >
            ...
            <div class="p-8">
                <!-- Title -->
                <h2 x-dialog:title class="text-2xl font-bold">Update Repack Date</h2>
                <span>{{$edit->rig_model}}</span>

                <!-- Content -->
                <x-group for="newRepackDate" label="New Repack Date" :error="$errors->first('newRepackDate')">
                    <x-date id="newRepackDate" wire:model="newRepackDate" field="newRepackDate" :error="$errors->first('newRepackDate')"/>
                </x-group>
                <div class="mt-2 mb-2 p-1 text-gray-900">Repack Due: {{$edit->reserve_due}} </div>
                <x-group label="Packed By" for="whoPacked">
                    <x-text wire:model="packedby" id="whoPacked" name="whopacked"  />
                </x-group>
            </div>
            ...
            <div class="p-4 flex justify-end space-x-2 bg-gray-50">
                ...
                <button type="submit" x-on:click="$dialog.close()"
